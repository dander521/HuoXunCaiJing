...
/** 导航条*/
@property (nonatomic, strong) DTNavigationViewController *nav;
...
- (void)showBrowseViewController
{
    ...
    self.nav = [[DTNavigationViewController alloc]initWithRootViewController:self];
    if([[[UIDevice currentDevice]systemVersion]floatValue] >= 8.0)
    {
        self.nav.modalPresentationStyle = UIModalPresentationOverCurrentContext;
    }
    _snapshotView = [rootViewController.view snapshotViewAfterScreenUpdates:NO];
    [rootViewController presentViewController:self.nav animated:NO completion:^{
...
- (void)viewDidLoad
{
    ...
    if (self.isHavaCustomBtn) {
        UIButton *customBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        customBtn.frame = CGRectMake((SCREEN_WIDTH-60)/2, SCREEN_HEIGHT-100, 60, 60);
        [customBtn setImage:[UIImage imageNamed:@"ic_main_goumai"] forState:UIControlStateNormal];
        customBtn.tag = kCustomButtonTag;
...
- (void)createBrowseView
{
    ...
    _countLabel.text = self.browseItemArray.count == 1 ? @"" : [NSString stringWithFormat:@"%ld/%ld",(long)_currentIndex + 1,(long)_browseItemArray.count];
...
- (void)tap:(MSSBrowseCollectionViewCell *)browseCell {
    ...
    {
        if ([self.view viewWithTag:kCustomButtonTag]) {
    ...
    if(browseItem.smallImageView)
    {
        ...
        [UIView animateWithDuration:0.5 animations:^{
            ...
            if ([self.view viewWithTag:kCustomButtonTag]) {
                [self.nav dismissViewControllerAnimated:NO completion:^{
...
- (void)dismissActionSheet:(NSNotification *)notify {
    [self performSelector:@selector(delayDisplay) withObject:nil afterDelay:0.2];
}

- (void)delayDisplay {
    if (self.isHavaCustomBtn) {
        UIButton *btn = (UIButton *)[self.view viewWithTag:kCustomButtonTag];
        btn.hidden = NO;
    }
}
