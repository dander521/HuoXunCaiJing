...
#import "TopToast.h"
//默认停留时间
#define ToastDispalyDuration 4.0f
//默认背景色
#define ToastBackgroundColor RGBA(0, 0, 0, 0.5)
...
-(void)showAnimationWithToast:(TopToast *)toast{
    ...
    toast.contentView.frame = CGRectMake(0, kTopHeight, [UIScreen mainScreen].bounds.size.width, toast.height);
    [UIView commitAnimations];
}
//影藏动画
-(void)hideAnimationWithToast:(TopToast *)toast{
    [UIView animateWithDuration:0.3 animations:^{
        toast.contentView.frame = CGRectMake(0, -toast.height*2, [UIScreen mainScreen].bounds.size.width, toast.height);
        ...
        if (_toastArr.count > 0) {
            ...
            if (_toastArr.count > 0) {
                [self showWithToast:_toastArr[0] view:nil];
            }
        }
    }];
}
...
- (void)hide{
    [UIView animateWithDuration:0.3 animations:^{
        _contentView.frame = CGRectMake(0, -_height*2, [UIScreen mainScreen].bounds.size.width, _height);
    }completion:^(BOOL finished) {
        [_contentView removeFromSuperview];
    }];
}

- (void)showWithToast:(TopToast *)toast view:(UIView *)view {
    if (view) {
        [view addSubview:toast.contentView];
        UIView *topView = [view viewWithTag:1000];
        NSArray *array = view.subviews;
        if (topView) {
            [view insertSubview:toast.contentView belowSubview:view];
        } else {
            [view bringSubviewToFront:toast.contentView];
        }
    } else {
...
+ (void)showToptoastWithText:(NSString *)text view:(UIView *)view{
    TopToast *toast = [[TopToast alloc] initWithText:text];
    [toast setDuration:ToastDispalyDuration];
    if (_toastArr.count == 0) {
        [toast showWithToast:toast view:view];
    }
...
+ (void)showToptoastWithText:(NSString *)text duration:(CGFloat)duration{
    TopToast *toast = [[TopToast alloc] initWithText:text];
    if (_toastArr.count == 0) {
        [toast showWithToast:toast view:nil];
    }
    [_toastArr addObject:toast];
}
