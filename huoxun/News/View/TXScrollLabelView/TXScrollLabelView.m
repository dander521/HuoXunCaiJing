...
#define TXScrollLabelFont [UIFont systemFontOfSize:13]
...
- (void)startup {
    ...
    if (_scrollType == TXScrollLabelViewTypeFlipRepeat || _scrollType == TXScrollLabelViewTypeFlipNoRepeat) {
        ...
        if (_scrollType == TXScrollLabelViewTypeFlipNoRepeat) {
            if ([NSString isTextEmpty:[self.scrollArray firstObject]]) {
                NSMutableAttributedString * attrStr = [TXCustomTools getAttributedStringWithLineSpace:@"暂无快讯" lineSpace:5 kern:0];
                [self setupAttributeTitle:attrStr];//初次显示
            } else {
                NSMutableAttributedString * attrStr = [TXCustomTools getAttributedStringWithLineSpace:[self.scrollArray firstObject] lineSpace:5 kern:0];
                [self setupAttributeTitle:attrStr];//初次显示
            }
        }
...
- (void)updateScrollText {
    ...
    NSMutableAttributedString * attrStrUp = [TXCustomTools getAttributedStringWithLineSpace:self.scrollArray[currentSentence] lineSpace:5 kern:0];
    self.upLabel.attributedText = attrStrUp;
    currentSentence ++;
    if (currentSentence >= self.scrollArray.count) currentSentence = 0;
    NSMutableAttributedString * attrStrDown = [TXCustomTools getAttributedStringWithLineSpace:self.scrollArray[currentSentence] lineSpace:5 kern:0];
    self.downLabel.attributedText = attrStrDown;
